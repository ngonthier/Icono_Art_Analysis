...
from keras.layers import Dense,Dropout,InputLayer,Layer,Activation,Input
...
class MIL_Dense(Layer):
    ...
    def __init__(self, units, 
        ...
        super(MIL_Dense, self).__init__(**kwargs)

    def build(self, input_shape):
        # Create a trainable weight variable for this layer.      

        assert len(input_shape) >= 3
        input_dim = input_shape[-1] # Feature dimensions
...
#        self.built = True
        super(MIL_Dense, self).build(input_shape)  # Be sure to call this at the end

    def call(self, x):
#        output = tf.einsum('ak,ijk->aij',self.kernel,x) # If multiple W at the same time
        output = tf.reduce_sum(tf.multiply(self.kernel,x),axis=2)
        if self.use_bias:
...
class MIL_Pooling(Layer):

    def __init__(self, pooling_mode,
                 **kwargs):
        self.pooling_mode = pooling_mode
        super(MIL_Pooling, self).__init__(**kwargs)

    def build(self, input_shape):
        # Create a trainable weight variable for this layer.
        assert len(input_shape) >= 3
        super(MIL_Pooling, self).build(input_shape)  # Be sure to call this at the end

    def call(self, x):
        output = K.max(x, axis=1, keepdims=False)
        return output

    def compute_output_shape(self, input_shape):
        return (input_shape[0],input_shape[-1])

class kerasMultiMIMAX():
    """
...
#        input_features = svm_X.shape[1]
        input_features = bags[0].shape[1]
        model = Sequential()
        data_input = Input(shape=(None,input_features),dtype='float32',name='input')
#        data_input = InputLayer(input_shape=(None,input_features),dtype='float32',name='input')
        scalar_Prod = MIL_Dense(number_neurones)(data_input)
        mil_pool = MIL_Pooling('max')(scalar_Prod)
        Tanh_after_mil_pool = Activation(tf.nn.tanh)(mil_pool)
        Tanh_before_mil_pool = Activation(tf.nn.tanh)(scalar_Prod)
        
        model = Model(inputs=[data_input], outputs=[Tanh_after_mil_pool])
        ...
        model.fit(bags, y, epochs=self.epochs,verbose=verboseNum,
                       class_weight=class_weights,validation_split=0,batch_size=batch_size,
                       shuffle=True)

        model_final = Model(inputs=[data_input], outputs=[Tanh_before_mil_pool])
...
#            iteration +=1
#            self.model.fit(bag, y, epochs=self.epochs,verbose=verboseNum,
#                       class_weight=class_weights,validation_split=0,batch_size=batch_size,
#                       shuffle=True)
#            all_labels = []
#            thres_pos_list = []
...
#        bags = [np.asmatrix(bag) for bag in bags]
        inst_preds_ = self.model.predict(bags)

        inst_preds = np.hstack(inst_preds_)
