...
based on early stopping idea to stabilize the prediction
"""
...
class ensDLearlyStop():
    ...
    def __init__(self,n_models=10,verbose=False,class_weight='balanced', **kwargs):
        """
...
#        list_size_layers = [64,128,64]
        ...
        def fit_model(trainX, trainy):
            ...
            for number_neurones in list_size_layers:
                model.add(Dense(number_neurones, activation=tf.nn.relu))
            model.add(Dense(1, activation=tf.nn.tanh))
            model.compile(optimizer=optimizer, 
              loss=loss, # binary_crossentropy < mean_squared_error < hinge : seems better
              metrics=[metric])
            model.fit(trainX, trainy, epochs=self.epochs,verbose=verboseNum,
        ...
        for i in range(self.n_models):
            #fit model
            if self.verbose: print('Train model :',i)
            model = fit_model(svm_X,svm_y)
            self.model += [model]
    ...
    def predict(self, bags, instancePrediction = None):
        ...
        all_pred = np.empty(shape=(np.vstack(bags).shape[0],self.n_models))
        for i in range(self.n_models):
            model = self.model[i]
            all_pred[:,i] = model.predict(np.vstack(bags)).ravel()

#        inst_preds = np.mean(all_pred,axis=1)
        inst_preds = np.max(all_pred,axis=1)
    ...
    def predict_proba(self, bags, instancePrediction = None):
        ...
        bags = [np.asmatrix(bag) for bag in bags]
        all_pred = np.empty(shape=(bags.shape[0],self.n_models))
        for i in range(self.n_models):
            model = self.model[i]
            all_pred[:,i] = model.predict(np.vstack(bags))
