...
from keras.layers import Dense,Dropout,InputLayer,Layer,Activation
...
#        self.built = True
    ...
    def call(self, x):
        output = K.einsum('ak,ijk->aij',self.kernel,x)
        if self.use_bias:
...
#                 kernel_initializer = 'Orthogonal',bias_initializer='RandomNormal',
    ...
    def fit(self, bags, y,epochs=1):
        ...
        model = Sequential()
        data_input = InputLayer(input_shape=(None,input_features,))
        scalar_Prod = MIL_Pooling(number_neurones)(data_input)
