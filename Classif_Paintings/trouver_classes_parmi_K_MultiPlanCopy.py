...
#                    'name_img' : tf.FixedLenFeature([],tf.string)}
    ...
    def parser_all_classes_wName_img(self,record):
        # Perform additional preprocessing on the parsed data.
#        keys_to_features={
#                    'height': tf.FixedLenFeature([], tf.int64),
#                    'width': tf.FixedLenFeature([], tf.int64),
...
#                    'name_img' : tf.FixedLenFeature([],tf.string)}
        ...
        tf.Print(name_img,[name_img])
        print('name_img',name_img)
        fc7 = tf.reshape(fc7, [self.num_rois,self.num_features])
        return fc7, label, name_img
    ...
    def tf_dataset_use_per_batch(self,train_dataset,listFilters=None):

        def filter_fn(*args,lF):
            return(tf.reduce_any(tf.equal(args[-1], lF)))
        
#        def filter_fn_partial(*args,listFilters):
##            print('x',x)
##            print('x',features)
##            print('train_dataset',train_dataset)
#            print(args)
##            name_img = tf.unstack(args[-1])
##            name_img = args[-1]
#            return(tf.reduce_any(tf.equal(args[-1], ['0','1'])))
##            return(tf.reshape(tf.reduce_any(tf.equal(name_img, ['0','1'])), []))
        
        if test_version_sup('1.6') and self.performance and (listFilters is None):
            ...
            # Cela retourne un <ParallelMapDataset shapes: ((100, 2), (1,), ()), types: (tf.float32, tf.float32, tf.string)>
            # Ce qui fait que le filter doit etre applique a cela, il va falloir 
            # TRouver un moyen de faire une fonction filter qui gere un nombre
            # different d' argument genre **arg ou un truc du genre !!!
            # TODO tu en es a la bouh
            if not(listFilters is None):
                from functools import partial
                filter_fn_partial = partial(filter_fn,lF=listFilters)
                train_dataset = train_dataset.filter(filter_fn_partial)
...
#            C_values =  np.logspace(-3,3,7,dtype=np.float32)
    ...
    def fit_MI_max_tfrecords(self,data_path,class_indice,shuffle=True,WR=False,
                             ...
                             storeLossValues=False,MultiPlan=2):
        """" 
        ...
        listFilters = [''] # List of the positives elements that you have to filters
        
        for index_plan in range(MultiPlan):
            # For loop on the new plan that we can create in the model
            # MultiPlan is the maximum number of Hyperplan
...
#                    self.first_parser = self.parser_all_classes
                    self.first_parser = self.parser_all_classes_wName_img
            else:
                if self.with_scores or self.seuillage_by_score or self.obj_score_add_tanh  or self.obj_score_mul_tanh:
            ...
            iterator_batch = self.tf_dataset_use_per_batch(train_dataset,listFilters)
            
            if self.with_scores or self.seuillage_by_score or self.obj_score_add_tanh  or self.obj_score_mul_tanh:
                X_batch,scores_batch, label_batch,name_img = iterator_batch.get_next()
            else:
                X_batch, label_batch, name_img = iterator_batch.get_next()
            ...
            # Define the dataset for the optimization loop
            if self.CV_Mode=='CV' or self.CV_Mode == 'CVforCsearch' :
                train_dataset_tmp = train_dataset_init.shard(self.num_split,0)
                for i in range(1,self.num_split-1):
            ...
            if test_version_sup('1.6') and shuffle and performance and len(listFilters)==0:
                ...
                train_dataset2 = train_dataset2.map(self.first_parser,
                                                num_parallel_calls=self.cpu_count)
#                train_dataset2 = train_dataset2.filter(lambda x: tf.reshape(tf.reduce_any(tf.equal(x[-1], listFilters)), []))
                if shuffle:
            ...
            if self.with_scores or self.seuillage_by_score or self.obj_score_add_tanh  or self.obj_score_mul_tanh:
                X_,scores_, y_, name_img_ = shuffle_iterator.get_next()
            else:
                X_, y_, name_img_ = shuffle_iterator.get_next()
