...
import nasnet_utils # Modified by Gonthier
...
if __name__ == '__main__':
  ...
  with tf.Graph().as_default():
     # The Inception networks expect the input image to have color channels scaled from [-1, 1]
      ...
      input_tensor = tf.placeholder(tf.float32, shape=(None,331,331,3), name='input_image')
      scaled_input_tensor = tf.scalar_mul((1.0/255), input_tensor)
      scaled_input_tensor = tf.subtract(scaled_input_tensor, 0.5)
      scaled_input_tensor = tf.multiply(scaled_input_tensor, 2.0)
      
      logits, end_points = build_nasnet_large(scaled_input_tensor,1000,is_training=False)
      saver = tf.train.Saver()
      saver.restore(sess, checkpoint_file)
         
      im = Image.open('cat.jpg').resize((331,331))
      im = np.array(im)
      im = im.reshape(-1,331,331,3)
