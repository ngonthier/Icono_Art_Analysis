...
from utils.save_param import create_param_id_file_and_dir
...
def my_config():
    ...
    seuil_estimation,thresh_evaluation,TEST_NMS = False,0.0,0.7
...
#    arrayParam = [demonet,database,N,extL2,nms_thresh,savedstr,mini_batch_size,performance,buffer_size,predict_with,shuffle,C,testMode,restarts,max_iters,CV_Mode,num_split,parallel_op,WR,norm,Optimizer,LR,optimArg,Number_of_positif_elt,number_zone,seuil_estimation,thresh_evaluation,TEST_NMS]
#    arrayParamStr = [demonet,database,N,extL2,nms_thresh,savedstr,mini_batch_size,performance,buffer_size,predict_with,shuffle,C,testMode,restarts,max_iters,'CV_Mode','num_split','parallel_op','WR','norm','Optimizer','LR','optimArg',Number_of_positif_elt,number_zone,seuil_estimation,thresh_evaluation,TEST_NMS]
#    param_name,path_data,file_param = create_param_id_file_and_dir(path_data,arrayParam,arrayParamStr)
    ...
    if parallel_op:
        ...
        if not os.path.isfile(cachefile_model) or ReDo:
             ...
             export_dir = classifierMILSVM.fit_MILSVM_tfrecords(data_path=data_path_train, \
                   class_indice=-1,shuffle=shuffle,WR=WR,performance=performance)
...
#                plt.xlim(min(bin_edges), max(bin_edges))
             if plot_hist:
                 ...
                 if seuil_estimation_debug:
                     ...
                     plt.axvline(result_full, color='red')
                     plt.legend(loc='best')
