...
def get_sets(data_path='/media/HDD/data/'):
  ...
  for year in ['2007', '2012']:
    for split in ['train', 'val', 'trainval', 'test']:
      ...
      __sets[name] = (lambda split=split, year=year: pascal_voc(split, year,devkit_path= data_path+'VOCdevkit',test_ext=True))
  ...
  for db in ['watercolor','comic','clipart']:
    for split in ['train', 'test']:
        ...
        __sets[name] = (lambda split=split, db=db: CrossMod_db(db,split,devkit_path=data_path+'cross-domain-detection/datasets',test_ext=True))
  ...
  for db in ['WikiTenLabels']:
    for split in ['test']:
        name = '{}_{}'.format(db,split)
        __sets[name] = (lambda split=split, db=db: WikiTenLabels_db(db,split,devkit_path=data_path+'Wikidata_Paintings/',test_ext=True))
