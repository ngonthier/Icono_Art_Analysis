...
def return_predict_gonthier(self, im):
    """
    Fonction created by Gonthier to have the confidence of the different classes
    """
    ...
    this_inp = np.expand_dims(im, 0)
    feed_dict = {self.inp : this_inp}
    #print(self.meta['out_size'],self.meta['classes'],self.meta['num']) # Nicolas Gonthier !!!
    out = self.sess.run(self.out, feed_dict)[0]
    #boxes = self.framework.findboxes(out)
    return out       
